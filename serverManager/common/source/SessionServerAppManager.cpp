                                                       ...
                                                       const firebolt::rialto::common::SessionServerState &newState)
{
    ...
    if (!sessionServer)
    {
        ...
        char telemetryBuff[128] = {0};
        snprintf(telemetryBuff, sizeof(telemetryBuff),
            "Change state of %s to %s failed - session server not found.", appName.c_str(),
                                        toString(newState));
        TELEMETRY_EVENT_STRING("Rialto Server Manager - SessionServerAppManager", telemetryBuff);
        return false;
    }
    ...
    if (!m_ipcController->performSetState(sessionServer->getServerId(), newState))
    {
        ...
        char telemetryBuff[128] = {0};
        snprintf(telemetryBuff, sizeof(telemetryBuff),
                                        "Change state of %s to %s failed.", appName.c_str(), toString(newState));
        TELEMETRY_EVENT_STRING("Rialto Server Manager - SessionServerAppManager", telemetryBuff);
...
bool SessionServerAppManager::configureSessionServerWithSocketName(const std::shared_ptr<ISessionServerApp> &kSessionServer)
{
                                                  ...
                                                  kSocketGroup, kAppName))
    {
        ...
        char telemetryBuff[128] = {0};
        snprintf(telemetryBuff, sizeof(telemetryBuff),
                                        "Configuration of server with id %d failed - ipc error.",
                                        kSessionServer->getServerId());
        TELEMETRY_EVENT_STRING("Rialto Server Manager - SessionServerAppManager", telemetryBuff);
        return false;
    }
