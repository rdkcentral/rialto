...
std::weak_ptr<firebolt::rialto::server::OcdmMock> m_ocdmMock;

OpenCDMError convertOpenCdmError(const firebolt::rialto::MediaKeyErrorStatus &status)
{
    switch (status)
    {
    ...
    {
        return OpenCDMError::ERROR_INVALID_SESSION;
    }
    case firebolt::rialto::MediaKeyErrorStatus::NOT_SUPPORTED:
    {
        return OpenCDMError::ERROR_KEYSYSTEM_NOT_SUPPORTED;
    }
    default:
    {
        return OpenCDMError::ERROR_UNKNOWN;
    }
    }
}

void setOcdmMock(const std::shared_ptr<firebolt::rialto::server::OcdmMock>& ocdmMock)
{
    m_ocdmMock = ocdmMock;
}
...
extern "C"
{
    ...
    { 
        auto ocdmMock = m_ocdmMock.lock();
        if (ocdmMock)
        {
            return convertOpenCdmError(ocdmMock->isTypeSupported(keySystem));
        }
        return ERROR_NONE; 
    }
