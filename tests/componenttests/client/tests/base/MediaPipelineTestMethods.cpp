...
{
    EXPECT_CALL(*m_mediaPipelineModuleMock,
                processAudioGap(_, processAudioGapRequestMatcher(kSessionId, kPosition, kDuration, kDiscontinuityGap),
                                _, _))
        .WillOnce(WithArgs<0, 3>(Invoke(&(*m_mediaPipelineModuleMock), &MediaPipelineModuleMock::defaultReturn)));
}
...
{
    EXPECT_TRUE(m_mediaPipeline->processAudioGap(kPosition, kDuration, kDiscontinuityGap));
}
...
{
    EXPECT_FALSE(m_mediaPipeline->processAudioGap(kPosition, kDuration, kDiscontinuityGap));
}
