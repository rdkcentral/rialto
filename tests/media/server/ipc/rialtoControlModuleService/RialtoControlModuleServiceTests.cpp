...
TEST_F(RialtoControlModuleServiceTests, shouldEstablishConnection)
{
    clientWillConnect();
    sendClientConnected();
}

TEST_F(RialtoControlModuleServiceTests, shouldGetSharedMemory)
{
    playbackServiceWillGetSharedMemory();
    sendGetSharedMemoryRequestAndReceiveResponse();
}

TEST_F(RialtoControlModuleServiceTests, shouldFailToGetSharedMemory)
{
