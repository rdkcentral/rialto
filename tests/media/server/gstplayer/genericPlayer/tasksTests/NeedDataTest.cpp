...
{
    firebolt::rialto::server::generic::NeedData task{m_context, &m_gstPlayerClient, &m_audioSrc};
    task.execute();
}

TEST_F(NeedDataTest, shouldDoNothingWhenVideoAppSourceIsNotPresent)
{
    firebolt::rialto::server::generic::NeedData task{m_context, &m_gstPlayerClient, &m_videoSrc};
    task.execute();
}
...
TEST_F(NeedDataTest, shouldDoNothingForUnknownAppSource)
{
    ...
    setupAppSource();
    firebolt::rialto::server::generic::NeedData task{m_context, &m_gstPlayerClient, &unknownSrc};
    task.execute();
}
