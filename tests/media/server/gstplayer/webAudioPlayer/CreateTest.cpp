...
TEST_F(RialtoServerCreateGstWebAudioPlayerTest, CreateDestroyLlamaSuccess)
{
    ...
    EXPECT_NO_THROW(m_gstPlayer = std::make_unique<GstWebAudioPlayer>(&m_gstPlayerClient, m_gstWrapperMock,
                                                                        m_glibWrapperMock, m_gstSrcFactoryMock, std::move(m_taskFactory),
...
TEST_F(RialtoServerCreateGstWebAudioPlayerTest, getGstSrcFailure)
{
    ...
    EXPECT_THROW(m_gstPlayer = std::make_unique<GstWebAudioPlayer>(&m_gstPlayerClient, m_gstWrapperMock,
                                                                        m_glibWrapperMock, m_gstSrcFactoryMock,
                                                                        std::move(m_taskFactory),
                                                                        std::move(workerThreadFactory),
                                                                        std::move(gstDispatcherThreadFactory)),
...
TEST_F(RialtoServerCreateGstWebAudioPlayerTest, createPipelineFailure)
{
    ...
    EXPECT_CALL(*m_gstWrapperMock, gstPipelineNew(_))
...
TEST_F(RialtoServerCreateGstWebAudioPlayerTest, createAppSrcFailure)
{
    ...
    EXPECT_CALL(m_workerThreadFactoryMock, createWorkerThread()).WillOnce(Return(ByMove(std::move(workerThread))));
    EXPECT_CALL(*m_gstWrapperMock, gstElementFactoryMake(_, _))
        .WillOnce(Return(nullptr));
...
/**
 * Test that a GstWebAudioPlayer throws an exception if there is a failure to create audio convert for rtkaudiosink.
 */
TEST_F(RialtoServerCreateGstWebAudioPlayerTest, createAudioConvertForRtkAudioSinkFailure)
{
    expectInitRialtoSrc();
    expectCreatePipeline();
    expectInitAppSrc();
...
/**
 * Test that a GstWebAudioPlayer throws an exception if there is a failure to create audio resample for rtkaudiosink.
 */
TEST_F(RialtoServerCreateGstWebAudioPlayerTest, createAudioResampleForRtkAudioSinkFailure)
{
    expectInitRialtoSrc();
    expectMakeRtkAudioSink();
    expectCreatePipeline();
