...
TEST(WorkerThreadTest, shouldEnqueueTaskAndExit)
{
    ...
    EXPECT_CALL(dynamic_cast<StrictMock<PlayerTaskMock> &>(*someTask), execute()).WillOnce(Invoke([&]() {
    ...
    std::unique_ptr<firebolt::rialto::server::IPlayerTask> shutdownTask{std::make_unique<StrictMock<PlayerTaskMock>>()};
    EXPECT_CALL(dynamic_cast<StrictMock<PlayerTaskMock> &>(*shutdownTask), execute()).WillOnce(Invoke([&]() {
        sut->stop();
    }));
