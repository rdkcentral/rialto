...
protected:
    ...
    {
        EXPECT_CALL(*m_gstWrapperMock, gstValueGetBuffer(&m_keyValue)).WillOnce(Return(&m_key));
        EXPECT_CALL(*m_gstWrapperMock, gstValueGetBuffer(&m_ivValue)).WillOnce(Return(&m_iv));
        EXPECT_CALL(*m_gstWrapperMock, gstValueGetBuffer(&m_subsamplesValue)).WillOnce(Return(&m_subsamples));
    }
};
...
TEST_F(RialtoServerDecryptorPrivateDecryptTest, SuccessEncrypted)
{
    ...
    EXPECT_CALL(*m_decryptionServiceMock,
                decrypt(m_keySessionId, &m_buffer, &m_subsamples, m_subsampleCount, &m_iv, &m_key, m_initWithLast15))
...
/**
 * Test GstRialtoDecryptorPrivate decrypt returns error when no subsamples set in the protection meta and subsamples
...
{
    expectGetInfoFromProtectionMeta();
    EXPECT_CALL(*m_gstWrapperMock, gstValueGetBuffer(&m_keyValue)).WillOnce(Return(nullptr));
...
TEST_F(RialtoServerDecryptorPrivateDecryptTest, InvalidIv)
{
    expectGetInfoFromProtectionMeta();
    EXPECT_CALL(*m_gstWrapperMock, gstValueGetBuffer(&m_keyValue)).WillOnce(Return(&m_key));
    EXPECT_CALL(*m_gstWrapperMock, gstValueGetBuffer(&m_ivValue)).WillOnce(Return(nullptr));
...
TEST_F(RialtoServerDecryptorPrivateDecryptTest, InvalidSubsamples)
{
    ...
    EXPECT_CALL(*m_gstWrapperMock, gstValueGetBuffer(&m_keyValue)).WillOnce(Return(&m_key));
    EXPECT_CALL(*m_gstWrapperMock, gstValueGetBuffer(&m_ivValue)).WillOnce(Return(&m_iv));
    EXPECT_CALL(*m_gstWrapperMock, gstValueGetBuffer(&m_subsamplesValue)).WillOnce(Return(nullptr));
