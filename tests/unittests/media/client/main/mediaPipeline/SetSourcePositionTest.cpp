...
protected:
    ...
    const uint64_t m_kRunningTime{3542};
...
TEST_F(RialtoClientMediaPipelineSetSourcePositionTest, Success)
{
    EXPECT_CALL(*m_mediaPipelineIpcMock,
                setSourcePosition(m_kSourceId, m_kPosition, m_kResetTime, m_kAppliedRate, m_kRunningTime))
        .WillOnce(Return(true));

    EXPECT_EQ(m_mediaPipeline->setSourcePosition(m_kSourceId, m_kPosition, m_kResetTime, m_kAppliedRate, m_kRunningTime),
              true);
}
