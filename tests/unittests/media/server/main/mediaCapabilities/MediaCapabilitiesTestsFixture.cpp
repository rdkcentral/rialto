...
MediaCapabilitiesTests::MediaCapabilitiesTests()
    : m_gstCapabilitiesMock(std::make_shared<StrictMock<firebolt::rialto::server::IGstCapabilitiesMock>>()),
      m_gstAudioCapabilities{"gst_aac", "gst_opus", {}},
      m_gstVideoCapabilities{"gst_h264", "gst_h265", {}}
{
    // Default: GStreamer returns its standard capabilities when queried
    EXPECT_CALL(*m_gstCapabilitiesMock, getSupportedAudioCapabilities())
        .WillRepeatedly(Return(m_gstAudioCapabilities));
    EXPECT_CALL(*m_gstCapabilitiesMock, getSupportedVideoCapabilities())
        .WillRepeatedly(Return(m_gstVideoCapabilities));

    // Create orchestrator with preloaded data
    m_mediaCapabilities = std::make_shared<firebolt::rialto::server::MediaCapabilities>(
...
void MediaCapabilitiesTests::gstCapabilitiesWillReturnEmptyAudio()
{
    ...
    EXPECT_CALL(*m_gstCapabilitiesMock, getSupportedAudioCapabilities())
        .WillOnce(Return(m_gstAudioCapabilities));
}
...
void MediaCapabilitiesTests::gstCapabilitiesWillReturnEmptyVideo()
{
    ...
    EXPECT_CALL(*m_gstCapabilitiesMock, getSupportedVideoCapabilities())
        .WillOnce(Return(m_gstVideoCapabilities));
}
