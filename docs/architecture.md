...
Rialto is partitioned into four architecture layers:

1. Public API layer
- `media/client/main` exposes playback, DRM, and web audio C++ APIs.

2. Domain IPC adapter layer
- `media/client/ipc`, `media/server/ipc`, `serverManager/ipc` adapt domain calls to/from protobuf RPC. 

3. Generic IPC transport layer
- `ipc/client`, `ipc/server`, `ipc/common` implement local transport primitives and fd passing.

4. Runtime orchestration and media execution layer
- `serverManager/*` controls session runtime lifecycle and health.
- `media/server/*` executes playback/DRM functionality. 
...
sequenceDiagram
    ...
    App->>Client: createMediaPipeline and register client
...
sequenceDiagram
    ...
    participant Session as media/server/ipc
    participant SessionMgr as session server manager

    SM->>Session: setConfiguration
    Session->>SessionMgr: apply limits and app metadata

    SM->>Session: setState(ACTIVE or INACTIVE)
    Session->>SessionMgr: transition session state

    SM->>Session: ping(id)
    Session-->>SM: AckEvent(id)
```

### Flow 3: Shared Memory Data Path
```mermaid
sequenceDiagram
    participant Client as media/client
    participant Session as media/server/ipc
    ...
    Client->>Linux: write media or web audio bytes
    Client->>Session: haveData or writeBuffer RPC
```
