...
public:
    MetricsCollector(int clientId, const std::shared_ptr<IMetricsCollectorClient> &client,
                     ...
                     ApplicationState initialApplicationState = ApplicationState::UNKNOWN);
...
private:
    ...
    static const char *webAudioPlayerStateToString(WebAudioPlayerState state);
    static const char *applicationStateToString(ApplicationState state);
    void notifyPlayerStateChanged(const std::string &context, const char *oldState, const char *newState,
                                  bool terminalState);
    ...
    std::optional<std::uint64_t> m_pendingPeriodicSampleId;
    unsigned int m_pendingPeriodicTimerCount{0};
    bool m_clientResponsive{true};

    std::mutex m_mutex;
    std::optional<PreviousSample> m_previousSample;

    // Per-player state tracking (typed context -> state)
    std::map<std::string, SessionMetricsState> m_sessionStates;
