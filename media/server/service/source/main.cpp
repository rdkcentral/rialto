...
int main(int argc, char *argv[])
{
    ...
    RIALTO_SERVER_LOG_MIL("(fz-dbg)tid:%ld from main.cpp calling Initialise\n", syscall(SYS_gettid));
    firebolt::rialto::server::IGstInitialiser::instance().initialise(&argc, &argv);
    RIALTO_SERVER_LOG_MIL("(fz-dbg)tid:%ld from main.cpp createApplicationSessionServer called\n", syscall(SYS_gettid));
    auto appSessionServer =
        firebolt::rialto::server::IApplicationSessionServerFactory::getFactory()->createApplicationSessionServer();
    RIALTO_SERVER_LOG_MIL("(fz-dbg)tid:%ld from main.cpp before init called\n", syscall(SYS_gettid));
    if (!appSessionServer->init(argc, argv))
    {
        RIALTO_SERVER_LOG_MIL("(fz-dbg)tid:%ld from main.cpp init failed\n", syscall(SYS_gettid));
        return EXIT_FAILURE;
    }
    RIALTO_SERVER_LOG_MIL("(fz-dbg)tid:%ld from main.cpp before startService called\n", syscall(SYS_gettid));
    appSessionServer->startService();
    RIALTO_SERVER_LOG_MIL("(fz-dbg)tid:%ld from main.cpp after startService called\n", syscall(SYS_gettid));
