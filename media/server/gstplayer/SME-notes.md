...
- Symptom → Cause → Fix
  - Audio underflow storm (`Audio stream underflow!` every 300 ms) → Audio decoder stopped consuming data; `lastAudioSampleTimestamps` frozen → Check for audio PTS rollback or large PTS gap in injected samples. Look for `[ADEC] Rollback PTS` lines from hardware decoder immediately before the stall. Verify `AttachSamples` is not being blocked waiting on `appsrc` max-bytes limit (8 MB video / 512 KB audio).
  ...
  - Decryptor element not injected (encrypted stream plays as garbled) → `GstDecryptorElementFactory` returned null → OpenCDM plugin not present in registry. Run `gst-inspect-1.0 | grep -i decrypt` on the target.
  - `unmapped underflow` warning from Rialto audio sink in client logs → Underflow event received by client-side sink for a session that has already been torn down or whose source ID is not found → Verify `invalidateActiveRequests` is called during `Flush` and that the session ID mapping is current. Often seen during rapid seek/ad-break transitions.
