...
{
    printf("(fz-dbg)tid:%ld inside GstCapabilitiesFactory.cpp createGstCapabilities\n", syscall(SYS_gettid));
    std::unique_ptr<IGstCapabilities> gstCapabilities;
    try
    {
        printf("(fz-dbg)tid:%ld inside GstCapabilitiesFactory.cpp calling getFactory for GstWrapper\n", syscall(SYS_gettid));
        ...
        printf("(fz-dbg)tid:%ld inside GstCapabilitiesFactory.cpp  about to call getGstWrapper\n", syscall(SYS_gettid));
        if ((!gstWrapperFactory) || (!(gstWrapper = gstWrapperFactory->getGstWrapper())))
        {
            printf("(fz-dbg)tid:%ld inside GstCapabilitiesFactory.cpp failed to get GstWrapper\n", syscall(SYS_gettid));
            throw std::runtime_error("Cannot create GstWrapper");
        }
        printf("(fz-dbg)tid:%ld inside GstCapabilitiesFactory.cpp successfully got GstWrapper\n", syscall(SYS_gettid));
