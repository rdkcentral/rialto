...
{
    printf("(fz-dbg)tid:%d inside GstInitialiser.cpp initialise method about to create a thread \n", syscall(SYS_gettid));
    if (m_thread.joinable())
    {
    ...
    m_thread = std::thread(
        [=]()
        {
            printf("(fz-dbg)tid:%d inside GstInitialiser.cpp thread getFactory called\n", syscall(SYS_gettid));
            std::shared_ptr<firebolt::rialto::wrappers::IGstWrapperFactory> factory =
                firebolt::rialto::wrappers::IGstWrapperFactory::getFactory();
            printf("(fz-dbg)tid:%d inside GstInitialiser.cpp getGstWrapper called\n", syscall(SYS_gettid));
            m_gstWrapper = factory->getGstWrapper();
            printf("(fz-dbg)tid:%d inside GstInitialiser.cpp after getGstWrapper\n", syscall(SYS_gettid));
            if (!m_gstWrapper)
            {
                RIALTO_SERVER_LOG_ERROR("Failed to create the gst wrapper");
                return;
            }
            printf("(fz-dbg)tid:%d inside GstInitialiser.cpp before gstInit\n", syscall(SYS_gettid));
            m_gstWrapper->gstInit(argc, argv);
            printf("(fz-dbg)tid:%d inside GstInitialiser.cpp after gstInit\n", syscall(SYS_gettid));
