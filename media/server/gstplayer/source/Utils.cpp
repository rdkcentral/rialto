...
const char *firstAudioFrameSignals[]{"first-audio-frame", "first-audio-frame-callback"};
...
std::optional<std::string> getFirstAudioFrameSignalName(const firebolt::rialto::wrappers::IGlibWrapper &glibWrapper,
                                                        GstElement *element)
{
    GType type = glibWrapper.gObjectType(element);
    guint nsignals{0};
    ...
    for (guint i = 0; i < nsignals; i++)
    {
        GSignalQuery query;
        glibWrapper.gSignalQuery(signals[i], &query);
        const auto signalNameIt = std::find_if(std::begin(firstAudioFrameSignals), std::end(firstAudioFrameSignals),
                                               [&](const auto *signalName)
                                               { return strcmp(signalName, query.signal_name) == 0; });

        if (std::end(firstAudioFrameSignals) != signalNameIt)
        {
            glibWrapper.gFree(signals);
            return std::string(*signalNameIt);
        }
