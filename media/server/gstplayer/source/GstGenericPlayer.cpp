...
void GstGenericPlayer::pushAdditionalSegmentIfRequired(GstElement *source)
{
    ...
    const bool allMatchCurrentPosition =
        !positions.empty() && std::all_of(positions.begin(), positions.end(), [&](const SegmentData &segment)
    ...
    if (allMatchCurrentPosition && positions.back().resetTime)
    {
        ...
        SegmentData additionalSegment = positions.back();
