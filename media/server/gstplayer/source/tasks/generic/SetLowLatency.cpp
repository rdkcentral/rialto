...
void SetLowLatency::execute() const
{
    ...
    if (sink->getSink())
    {
        if (m_glibWrapper->gObjectClassFindProperty(G_OBJECT_GET_CLASS(sink->getSink()), "low-latency"))
        {
            m_glibWrapper->gObjectSet(sink->getSink(), "low-latency", m_lowLatency, nullptr);
        }
        else
        {
            RIALTO_SERVER_LOG_ERROR("Failed to set low-latency property on sink '%s'", GST_ELEMENT_NAME(sink.get()));
        }
    }
    else
    {
