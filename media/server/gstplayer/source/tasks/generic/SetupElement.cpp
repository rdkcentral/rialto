    ...
    : m_context{context}, m_gstWrapper{gstWrapper}, m_glibWrapper{glibWrapper}, m_player{player}, m_element{element}
{
    ...
    if (isVideoParser(*m_gstWrapper, m_element))
    {
        ...
        bool streamSyncModePending{false};
        {
            ...
            streamSyncModePending =
                m_context.pendingStreamSyncMode.find(MediaSourceType::VIDEO) != m_context.pendingStreamSyncMode.end();
        }
    ...
    else if(isAudioDecoder(*m_gstWrapper, m_element))
    {
        ...
        bool bufferingLimitPending{false};
        {
            std::unique_lock lock{m_context.propertyMutex};
            streamSyncModePending =
                m_context.pendingStreamSyncMode.find(MediaSourceType::AUDIO) != m_context.pendingStreamSyncMode.end();
